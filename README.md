...
- 원본 응답은 `.cache/krx-daily-v2-{시작일}-{종료일}.json`에 캐시됩니다(git 무시).
  같은 기간으로 다시 실행하면 네트워크를 타지 않고 즉시 끝납니다.
  파일명의 `v2`는 캐시 스키마 버전이라, 수집 항목이 바뀌면 올려서 옛 캐시를 자연히 버립니다.
...
- **5일 이동평균선만** 그립니다. 수집 범위가 1개월(약 21영업일)이라 20일선은 점이 몇 개
  안 나오고 60·120일선은 아예 불가능합니다. 없는 걸 그리느니 하나를 정확히 그렸습니다
